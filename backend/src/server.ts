...
import WebSocket from 'ws'; // Keep this for the global WebSocket if you need the client side type, but not for Server
import { WebSocketServer } from 'ws'; // <-- CORRECT IMPORT FOR THE SERVER
...
import path from 'path'; // <-- ADDED: Path module for serving static files

// Importez les nouveaux modèles
import { Session, ISession, ISessionMember, IStoryInSession, IVote } from './models/Session';
...
const wss = new WebSocketServer({ server }); // <-- FIXED: Use WebSocketServer from 'ws'

// Connexion à MongoDB
// Assurez-vous que votre .env a MONGODB_URI, ou utilisez MONGO_URI si c'est ce que vous avez configuré
mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/planning_poker_db')
...
app.post('/api/sessions/join', authenticateToken, async (req, res) => {
    const { sessionId } = req.body;
...
app.post('/api/sessions/:sessionId/stories', authenticateToken, async (req, res) => {
    const { sessionId } = req.params;
    ...
    try {
        ...
        const newStoryInSession: IStoryInSession = {
...
app.post('/api/sessions/:sessionId/selectStory', authenticateToken, async (req, res) => {
    const { sessionId } = req.params;
...
app.delete('/api/sessions/:sessionId/stories/:storyInSessionId', authenticateToken, async (req, res) => {
...
const sessionClients = new Map<string, WebSocketWithSession[]>();

wss.on('connection', async (ws: WebSocketWithSession, req) => {
    ...
    try {
        ...
        ws.userId = new mongoose.Types.ObjectId(decoded.userId);
    ...
    ws.on('message', async (message: string | Buffer) => { // <-- Changed message type to allow Buffer
        ...
        try {
            ...
            const session = await Session.findOne({ sessionId: ws.sessionId });
            if (!session) return;
            ...
            switch (type) {
                case 'vote':
                    if (currentStoryInSession && !currentStoryInSession.revealed) {
                        const existingVoteIndex = currentStoryInSession.votes.findIndex(
                            v => v.userId.equals(ws.userId)
                        );
                ...
                case 'resetme':
                    if (currentStoryInSession) {
                        currentStoryInSession.votes = currentStoryInSession.votes.filter(
                            v => !v.userId.equals(ws.userId)
                        );
                        await session.save();
                        broadcastSessionUpdate(session);
...
const formatSessionForClient = (session: ISession, requestingUserId: mongoose.Types.ObjectId) => {
    ...
    sessionCopy.stories = sessionCopy.stories.map((story: IStoryInSession) => { // <-- Explicitly type 'story'
        if (!story.revealed) {
            story.votes = story.votes.map((vote: IVote) => ({ // <-- Explicitly type 'vote'
                ...vote,
    ...
    return {
        ...
        members: sessionCopy.members.map((m: ISessionMember) => ({ // <-- Explicitly type 'm'
            username: m.username,
            userId: m.userId?.toString() // Use optional chaining just in case, though it should exist
        })),
        stories: sessionCopy.stories.map((story: IStoryInSession) => ({ // <-- Explicitly type 'story' again for safety
            _id: story._id?.toString(), // Convert ObjectId to string
            storyId: story.storyId?.toString(), // Convert ObjectId to string
            title: story.title,
            description: story.description,
            tasks: story.tasks,
            votes: story.votes.map((vote: IVote) => ({ // <-- Explicitly type 'vote' again for safety
...
const broadcastSessionUpdate = async (session: ISession) => {
